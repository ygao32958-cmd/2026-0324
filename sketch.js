...
const SEAWEED_HEIGHT_RATIO = 0.3; // 水草占30%高度（原本60%的一半）
...
function drawSeaweed() {
  const seaweedCount = 10; // 水草數量
  const width = 40; // 增加寬度
  
  for (let s = 0; s < seaweedCount; s++) {
    const baseX = map(s, 0, seaweedCount - 1, width * 1.5, windowWidth - width * 1.5);
    drawSingleSeaweed(baseX, width, s);
  }
}

function drawSingleSeaweed(baseX, width, index) {
  const seaweedHeight = windowHeight * SEAWEED_HEIGHT_RATIO;
